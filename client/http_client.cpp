...
int HTTPClient::splitURL(const std::string *url, std::string *ptcl, std::string *addr, std::string *file, int *port)
{
...
uint32_t HTTPClient::receive_by_line(TCPClient *client, char *buf, uint32_t buf_size)
{
    bool isHead = true;
    uint32_t received = 0;
    ...
    while (received < buf_size) {
        ...
        if (temp == '\n') {
            // split head and body
            if (isHead && ((buf[received - 4] == buf[received - 2]) && buf[received - 4 == '\r'] && buf[received - 3] == '\n')) {
                isHead = false;

                std::string _headerStr(buf, received - 2);
                std::string _headerStrLf = NewLineCode::convert(_headerStr, NewLineCode::LF);

                HTTPHeader _header;
                _header.set(_headerStrLf);

                if (_header.get(std::string("Transfer-Encoding")) == std::string("chunked")) {
                    chunk = true;
                } else {
...
int HTTPClient::send_and_recive(std::string *addr, int port, std::string *mesg, std::string *recv_str)
{
...
int HTTPClient::split_header_body(std::string *_src)
{
    ...
    if (recv_header->get(std::string("Transfer-Encoding")) == std::string("chunked")) {
...
int HTTPClient::post(const std::string *url, const std::string *post, HTTPHeader *header)
{
    std::string *ptcl = new std::string();
    std::string *addr = new std::string();
    std::string *file = new std::string();
...
std::string *HTTPClient::status()
{
    return recv_status;
}
HTTPHeader *HTTPClient::header()
{
    return recv_header;
}
