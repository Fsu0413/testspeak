...
// for debug use only
#ifdef __DEBUG__
int TCPClient::get_error_no()
{
    return _err_no;
}
std::string TCPClient::get_error_mesg()
{
    return _err_mesg;
}
#endif
...
int hostname2ipaddr(const char *hostname, char *ipaddr)
{
    ...
    int ret = getaddrinfo(hostname, NULL, NULL, &response);
    if (ret != 0) {
        fprintf(stderr, "Cannot convert host to IP address. err=%s\n", gai_strerror(ret));
        return -1;
    }
