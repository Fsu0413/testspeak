...
//template <class T> class DeletePtr : public std::unary_function<void, T>
//{
//public:
...
//            progress->signal_finished().connect(sigc::bind(sigc::mem_fun(*this, &Application::on_progress_finished), progress));
//        }
//    } catch (...) {
...
//    std::for_each(progress_threads_.begin(), progress_threads_.end(), std::mem_fun(&ThreadProgress::launch));
//}
...
//    if (std::find_if(progress_threads_.begin(), progress_threads_.end(), std::mem_fun(&ThreadProgress::unfinished)) == progress_threads_.end()) {
//        main_loop_->quit();
//    }
//}
//} // anonymous namespace
